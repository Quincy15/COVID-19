...
newdata<-data.frame(data$Date,data$xyqz,data$rate)
...
# ????ģ??ͼ
p1<-ggplot(sanyue,aes(x=sanyue$data.Date))+scale_x_date(labels=date_format("%d/%m"))+
...
# ????Ԥ??ֵ
for(i in r_length:L){
  newdata3[i+1,6]<-(1+newdata3$fit[i+1])*newdata3[i,6]# ????Ԥ??ֵ
  newdata3[i+1,7]<-(1+newdata3$lwr[i+1])*newdata3[i,7]# ????????
  newdata3[i+1,8]<-(1+newdata3$upr[i+1])*newdata3[i,8]# ????????
}
...
p3<-ggplot(newdata3,aes(x=t,y=observations),color="black")+
  ...
  geom_text(mapping=aes(x=t[r_length+1], y=30000, label=t[r_length+1]),size=3, 
            angle=90, vjust=-0.5, hjust=-0.2, col="red")+
  geom_vline(xintercept = t[point_fit],color="pink",linetype="dotted")+
  geom_text(mapping=aes(x=t[point_fit+5], y=90000, label=t[point_fit]),size=3, 
            angle=90, vjust=-0.5, hjust=-0.2, col="red")+
